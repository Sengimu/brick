...
public class ProfileController {
    ...
    public Res getProfileInfoByUUID(@PathVariable("uuid") String uuid, @RequestParam(name = "unsigned", required = false) String unsigned) {

        ProfileInfo profileInfo = profileInfoService.getProfileInfoByUUID(uuid);
        return null != profileInfo ? Res.ySuccess(profileInfo) : Res.ySuccess(204);
    }

    @PostMapping("/api/profiles/minecraft")
    public Res getProfileInfosByName(@RequestBody List<String> params) {
    ...
    public Res putTextureByUUID(@PathVariable("uuid") String uuid, @PathVariable("textureType") String textureType, @RequestParam("model") String model, @RequestParam("file") MultipartFile file, @RequestHeader(name = "Authorization", required = false) String authorization) {

        if (null == authorization || null == tokenService.validateToken(authorization.split(" ")[1])) {
        ...
        try {
            ...
            if (width % 32 != 0 || width > 512 || height % 32 != 0 || height > 512) {
                throw new RuntimeException("The file is illegal.");
            }
        } catch (IOException e) {
            return Res.yFail("ForbiddenOperationException", "The file is illegal.", 403);
        }
        ...
        try {
            hashPath = DigestUtil.sha256Hex(file.getBytes());
